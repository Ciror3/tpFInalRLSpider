...
from tpFInalRLSpider.spiderJAR.enviroment_previous_steps import ACTION_METADATA, SpiderEnv


def parse_args():
    parser = argparse.ArgumentParser(
        description="Entrena PPO usando el environment con movimiento previo en la observacion."
    )
    parser.add_argument(
        "-n",
        "--total-timesteps",
    ...
    parser.add_argument(
        ...
        type=int,
        default=4,
        help="cantidad de environments paralelos",
    )
    parser.add_argument(
        "--seed",
        type=int,
        default=0,
        help="semilla base para los environments",
    )
    parser.add_argument(
        "--device",
        default="auto",
    ...
    parser.add_argument(
        "--calibration-path",
        default=None,
        help="ruta opcional al archivo calibration_results_combinations_full.txt",
    )
    parser.add_argument(
        "--no-previous-action",
        action="store_true",
    ...
    parser.add_argument(
        ...
        type=int,
        default=5_000,
        help="frecuencia de guardado de checkpoints en timesteps globales",
    )
    parser.add_argument(
        "--policy-map-freq",
        type=int,
        default=20_000,
        help="frecuencia de guardado de mapas de politica en timesteps globales",
    )
    parser.add_argument(
        "--policy-map-limit",
        type=float,
        default=2.0,
        help="limite del mapa de politica: evalua x,y en [-limit, limit]",
    )
    parser.add_argument(
        "--policy-map-step",
        type=float,
        default=0.1,
        help="separacion entre puntos del mapa de politica",
    )
...
class PreviousStepsPolicyMapCallback(BaseCallback):
    def __init__(
        self,
        freq=20_000,
        save_path="policy_maps_previous_steps",
        env_kwargs=None,
        map_limit=2.0,
        map_step=0.1,
        verbose=1,
    ...
    def _on_step(self) -> bool:
        ...
        for previous_motion in previous_motions:
            ...
            plt.title(f"Policy map - prev={previous_label} - step {self.num_timesteps}")
...
def main():
    ...
    models_dir = os.path.join("models_previous_steps", run_name)
    policy_maps_dir = os.path.join("policy_maps_previous_steps", run_name)
    ...
    checkpoint_cb = CheckpointCallback(
        ...
        name_prefix="ppo_spider_previous_steps",
    )
    policy_map_cb = PreviousStepsPolicyMapCallback(
    ...
    try:
        ...
        final_model_path = os.path.join(models_dir, "ppo_spider_final_previous_steps")
